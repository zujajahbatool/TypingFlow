...
function onTextChanged(event: vscode.TextDocumentChangeEvent) {
  ...
  const ignoredSchemes = ["output", "debug", "vscode", "git",
                          "extension-output", "search-editor"];
  const ignoredLangs   = ["Log", "log", "code-runner-output",
                           "extension-output"];

  if (ignoredSchemes.includes(scheme)) { return; }
  if (ignoredLangs.includes(lang))     { return; }
  if (event.document.uri.scheme !== "file" &&
      event.document.uri.scheme !== "untitled") { return; }
  // ──────────────────────────────────────────────────────────────────
  ...
  for (const change of event.contentChanges) {
    ...
    if (text.includes(" ") || text.includes("\n")) {
  	...
  	if (isAutocomplete) {
  	  ...
  	  log(`Autocomplete detected — "${text.slice(0,20)}..." counted as 1 keystroke`);
  	}
  }
...
function updateWPM(now: number) {
  ...
  const rawWPM        = Math.round(
    (recentKeys.length / AVG_WORD_LENGTH) / windowMinutes
  );

  // ── Cap at realistic human maximum (220 WPM) ──────────────────────
  const wpm = Math.min(rawWPM, 220);
  // ──────────────────────────────────────────────────────────────────

  state.currentWPM = wpm;
  if (wpm > state.burstWPM) { state.burstWPM = wpm; }

  if (wpm > 0) {
    state.wpmHistory.push(wpm);
    if (state.wpmHistory.length > 20) { state.wpmHistory.shift(); }
  }
}

// ── METRICS ───────────────────────────────────────────────────────────────────
function getConsistency(): number {
  if (state.wpmHistory.length < 3) { return 0.5; }
  const mean = state.wpmHistory.reduce((a, b) => a + b, 0) / state.wpmHistory.length;
  const variance = state.wpmHistory.reduce(
    (s, v) => s + Math.pow(v - mean, 2), 0
  ) / state.wpmHistory.length;
  return Math.max(0, Math.min(0.99, 1 - Math.sqrt(variance) / 30));
}
...
function getContext(): string {
  ...
  const codeLanguages = ["python","javascript","typescript","java",
                         "cpp","c","go","rust","php","ruby"];
  if (codeLanguages.includes(lang)) { return "coding"; }
...
async function checkIfThinkingPause(pauseSecs: number) {
  ...
  try {
    const result = await apiPost("/predict/pause", payload) as any;
    if (result) {
      state.isThinkingPause = result.is_thinking;
      updatePauseUI(result.is_thinking);
      log(`Pause: ${result.pause_label} (${(result.confidence*100).toFixed(0)}%)`);
    }
  } catch (e) {
...
async function saveSession() {
  ...
  try {
    const result = await apiPost("/session/save", payload) as any;
    if (result) { log(`Saved — ${result.message}`); }
  } catch (e) {
...
function updateStatusBar() {
  ...
  statusBarItem.backgroundColor = wpm >= 80
...
function updatePauseUI(isThinking: boolean) {
  if (isThinking) {
    ...
    pauseStatusItem.tooltip         = "TypingFlow AI detected a productive thinking pause";
...
async function showStatsPanel() {
  ...
  try {
    ...
    if (rank) { rankMsg = rank.message; }
  } catch { rankMsg = "Backend offline"; }
...
function log(msg: string) {
  outputChannel.appendLine(`[TypingFlow] ${new Date().toLocaleTimeString()} — ${msg}`);
}
